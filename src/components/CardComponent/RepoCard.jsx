    ...
    <div className="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 duration-300 flex flex-col my-5 z-10 justify-between max-sm:p-3 rounded-lg h-48 ">
      ...
      <div className="flex m-0 p-0 text-white flex-col">
        ...
        <div className="flex flex-col text-white bg-gray-800/95 justify-between text-left h-36 px-2">
          <div className="pt-2 text-sm font-customFont">
            <p>{repo.description}</p>
          </div>
          <p className="font-medium text-sm">Último commit em {date}</p>
        </div>
      </div>
      <div>
        <div className="flex text-white bg-gray-800/95  rounded-b-lg items-center justify-between">
          ...
          <div className="flex mr-2 items-center space-x-3">
            <a href={repo.html_url} target="_blank" rel="noopener noreferrer">
              <GithubIcon height={20} width={20} className="hover:pointer hover:animate-pulse" />
            </a>
            {repo.homepage && (
              <a href={repo.homepage} target="_blank" rel="noopener noreferrer">
                <LinkIcon height={20} width={20} className="hover:pointer hover:animate-pulse" />
              </a>
            )}
          </div>
        </div>
